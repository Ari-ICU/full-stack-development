...
export default function Home() {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-[#050a05] flex flex-col font-sans selection:bg-green-500/30">
      {/* Background Decorative Elements */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute inset-0 opacity-[0.03] bg-[radial-gradient(circle_at_20%_80%,#339933_2px,transparent_2px),radial-gradient(circle_at_80%_20%,#339933_1px,transparent_1px)] bg-[length:50px_50px]" />
        ...
        <motion.div 
          ...
          className="absolute -top-[100px] -right-[100px] w-[600px] h-[600px] bg-green-500/20 blur-[120px] rounded-full" 
        />
        <motion.div 
          animate={{ 
            x: currentSlide * 40,
          ...
          transition={{ duration: 1 }}
          className="absolute bottom-[100px] -left-[50px] w-[500px] h-[500px] bg-emerald-600/10 blur-[120px] rounded-full" 
        />
      </div>
            ...
            {[0, 1].map((i) => (
              <button
                ...
                className={`h-1.5 transition-all duration-300 rounded-full ${
                  currentSlide === i ? 'w-8 bg-green-500' : 'w-2 bg-white/20 hover:bg-white/40'
                }`}
              />
            ))}
          </div>
          ...
          <>
            <div className="absolute top-1/2 -translate-y-1/2 left-4 md:left-10 z-20">
              <Button 
                ...
                size="icon" 
                onClick={prevSlide}
                className="w-14 h-14 rounded-full border border-white/10 bg-white/5 backdrop-blur-md hover:bg-white/10 hover:border-green-500/50 text-white transition-all duration-300 group"
              >
