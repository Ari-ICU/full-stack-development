...
export default function Home() {
    ...
    return (
      <div className="h-screen w-full bg-slate-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-2 border-orange-500/10 border-t-orange-500 rounded-full animate-spin" />
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-slate-50 flex flex-col font-sans selection:bg-orange-500/10 text-slate-900">
      {/* Background Decorative Elements */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute inset-0 opacity-[0.05] bg-[radial-gradient(circle_at_20%_80%,#e34c26_1px,transparent_1px),radial-gradient(circle_at_80%_20%,#e34c26_1px,transparent_1px)] bg-[length:60px_60px]" />
        ...
        <motion.div 
          ...
          className="absolute -top-[100px] -right-[100px] w-[600px] h-[600px] bg-orange-500/5 blur-[120px] rounded-full" 
        />
        <motion.div 
          animate={{ 
            x: currentSlide * 40,
          ...
          transition={{ duration: 1 }}
          className="absolute bottom-[100px] -left-[50px] w-[500px] h-[500px] bg-amber-600/5 blur-[120px] rounded-full" 
        />
      </div>
            ...
            {[0, 1].map((i) => (
              <button
                ...
                className={`h-1.5 transition-all duration-300 rounded-full ${
                  currentSlide === i ? 'w-10 bg-orange-500 shadow-md shadow-orange-100' : 'w-2.5 bg-slate-200 hover:bg-slate-300'
                }`}
              />
            ))}
          </div>
          ...
          <>
            <div className="absolute top-1/2 -translate-y-1/2 left-4 md:left-10 z-20">
              <Button 
                ...
                size="icon" 
                onClick={prevSlide}
                className="w-14 h-14 rounded-full border border-slate-200 bg-white shadow-sm hover:bg-slate-50 hover:border-orange-500/30 text-slate-400 hover:text-slate-900 transition-all duration-300 group"
              >
      ...
      {/* Footer Section - Only show on Hero and Curriculum slides */}
